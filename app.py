...
def check(email):
    url = "https://mges724v7j.execute-api.us-east-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://mges724v7j.execute-api.us-east-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://mges724v7j.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    print(address)
    url = "https://mges724v7j.execute-api.us-east-1.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    ...
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=ny96MqSv2JhHeVbZgi3DY7fad4topEKFTrmOXWzwuUI0Rl5CAPhpLk3aq4fmHwnxtvUgZYsiAE2Il0Xb&sender_id=FSTSMS&message="+msg+"&language=english&route=p&numbers="+str(i['phone'])
